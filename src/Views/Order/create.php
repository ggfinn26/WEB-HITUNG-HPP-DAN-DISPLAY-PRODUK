<div class="glass p-8 md:p-12 rounded-3xl shadow-2xl max-w-3xl mx-auto border-t-4 border-t-primary relative overflow-hidden">
    ...
    <div class="mb-10 text-center relative z-10">
        <span class="inline-block px-3 py-1 mb-3 rounded-full bg-primary/10 text-primary text-xs font-bold tracking-wider uppercase">Order Baru</span>
        <h1 class="text-3xl md:text-4xl font-black text-text tracking-tight mb-2">Formulir Pesanan</h1>
        <p class="text-muted">Isi data pelanggan dengan lengkap untuk memproses order.</p>
    </div>
    ...
    <form action="?page=orders&action=store" method="POST" class="space-y-6 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2 group">
                <label for="namaPemesan" class="block text-sm font-semibold text-text group-focus-within:text-primary transition-colors">Nama Lengkap</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-muted group-focus-within:text-primary transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                    </div>
                    <input type="text" id="namaPemesan" name="namaPemesan" required placeholder="John Doe"
            ...
            <div class="space-y-2 group">
                <label for="whatsappPemesan" class="block text-sm font-semibold text-text group-focus-within:text-secondary transition-colors">No. WhatsApp</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-muted group-focus-within:text-secondary transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                    </div>
                    <input type="text" id="whatsappPemesan" name="whatsappPemesan" required placeholder="08123456789"
        ...
        <div class="space-y-2 group">
            <label for="instagramUserNamePemesan" class="block text-sm font-semibold text-text group-focus-within:text-pink-500 transition-colors">Username Instagram <span class="text-muted font-normal">(Opsional)</span></label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-muted group-focus-within:text-pink-500 transition-colors">
                    <span class="font-bold">@</span>
                </div>
                <input type="text" id="instagramUserNamePemesan" name="instagramUserNamePemesan" placeholder="username"
        ...
        <div class="space-y-2 group">
            <label for="alamatPemesan" class="block text-sm font-semibold text-text group-focus-within:text-primary transition-colors">Alamat Pengiriman</label>
            <textarea id="alamatPemesan" name="alamatPemesan" rows="3" required placeholder="Jl. Sudirman No. 123..."
                      class="w-full px-4 py-3 rounded-2xl border border-outline-variant bg-surface focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all shadow-sm resize-none"></textarea>
        </div>
        ...
        <div class="pt-6 mt-6 border-t border-outline-variant">
            <div class="flex justify-between items-center mb-4">
                ...
                <button type="button" id="addItemBtn" class="px-4 py-2 bg-secondary/10 text-secondary font-bold rounded-xl hover:bg-secondary/20 transition-colors text-sm flex items-center gap-1">
        ...
        <div class="pt-8 mt-8 border-t border-outline-variant flex flex-col-reverse sm:flex-row justify-end gap-4">
            <a href="?page=orders" class="px-6 py-3.5 rounded-2xl border border-outline-variant text-text hover:bg-background transition-colors font-bold text-center">Batal</a>
            <button type="submit" class="px-8 py-3.5 rounded-2xl bg-primary text-white shadow-lg shadow-primary/30 hover:shadow-xl hover:-translate-y-0.5 transition-all font-bold flex items-center justify-center gap-2">
                Simpan Pesanan
...
(function () {
    ...
    function calculateTotal() {
        ...
        if (marginEl) {
            ...
            marginEl.className   = margin >= 0 ? 'text-xl font-black text-green-600' : 'text-xl font-black text-red-500';
        }
    }

    addItemBtn.addEventListener('click', () => { container.appendChild(createItemRow()); calculateTotal(); });
