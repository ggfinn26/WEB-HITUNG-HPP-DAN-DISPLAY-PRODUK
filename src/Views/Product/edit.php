    ...
    <div class="glass-panel p-5 sm:p-6 md:p-10 rounded-3xl shadow-lg border border-outline-variant/30 max-w-4xl">
        <form action="?page=products&action=update&id=<?= $product->getId() ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div id="quill-editor" class="bg-surface-container border-2 border-outline-variant/50 rounded-xl" style="height: 150px;"><?= $product->getDescription() ?? '' ?></div>
...
<?php
    $jsGroups = [];
    if (isset($variantGroups) && isset($variantOptions)) {
        foreach ($variantGroups as $g) {
            $opts = [];
            if (isset($variantOptions[$g->getId()])) {
                foreach ($variantOptions[$g->getId()] as $o) {
                    $opts[] = $o->getName();
                }
            }
            $jsGroups[] = [
...
?>
<script>
    window.INITIAL_VARIANT_GROUPS = <?= json_encode($jsGroups) ?>;
    window.INITIAL_VARIANTS = <?= json_encode($jsVariants) ?>;
    window.INITIAL_VARIANT_IMAGES = <?= json_encode($jsImages) ?>;
</script>
